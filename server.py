...
def generate_confidence_graph(confidence):
    try:
        ...
        sizes = [confidence, 100 - confidence]  # Real, Fake
        labels = ['Real', 'Fake']
        explode = (0.05, 0)
        
        plt.figure(figsize=(2, 2))  # ↓↓↓ This line makes it smaller
...
def detect():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Set output label
            output = "FAKE" if prediction_class == 0 else "REAL"

            # Adjust confidence for correct graph proportions
            confidence_for_graph = confidence if output == "REAL" else 100 - confidence

            # Generate graphs
            confidence_image = generate_confidence_graph(confidence_for_graph)
